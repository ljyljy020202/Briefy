...
from app.core.llm_client import LLMClientError, LLMTokenUsage, LLMUnavailableError
...
def _quality(has_desc: bool = True, has_roles: bool = False, trunc: bool = False, length: int = 100) -> ClassifyInputQuality:
...
def _request(*postings: ClassifyPostingInput, req_id: str = "req-1", version: str = "1.0.0") -> ClassifyRequest:
...
class TestNonItClassification:
    ...
    async def test_sales_classified_as_non_it(self):
        ...
        llm = _mock_llm(_llm_batch_response(
            _llm_result(1, domain="NON_IT", scope="ROLE_SPECIFIC",
                        role_groups=["NON_DEV"], recruitment_type="EXPERIENCED_HIRE",
                        evidence="영업 담당자")
        ))
    ...
    async def test_insurance_admin_classified_as_non_it(self):
        ...
        llm = _mock_llm(_llm_batch_response(
            _llm_result(2, domain="NON_IT", role_groups=["NON_DEV"], evidence="보험 총무 담당")
        ))
        svc = _make_service(llm)
        resp = await svc.classify(_request(p))
    ...
    async def test_operations_manager_classified_as_non_it(self):
        p = _posting(3, "Operations Manager", description="Manage daily ops and compliance")
        llm = _mock_llm(_llm_batch_response(
            _llm_result(3, domain="NON_IT", role_groups=["NON_DEV"], evidence="Operations Manager")
        ))
        svc = _make_service(llm)
        resp = await svc.classify(_request(p))
        assert resp.results[0].job_domain == "NON_IT"
...
class TestAmlDistinction:
    ...
    async def test_aml_manager_is_non_it(self):
        ...
        llm = _mock_llm(_llm_batch_response(
            _llm_result(10, domain="NON_IT", role_groups=["NON_DEV"],
    ...
    async def test_aml_backend_developer_is_backend(self):
        p = _posting(11, "AML Backend Developer",
                     description="AML 시스템 백엔드 API 개발 (Java Spring Boot)")
        llm = _mock_llm(_llm_batch_response(
            _llm_result(11, domain="IT", role_groups=["BACKEND"],
...
class TestItRoleDistinction:
    ...
    async def test_backend_and_frontend_in_same_request_distinct(self):
        ...
        llm = _mock_llm(_llm_batch_response(
            ...
            _llm_result(22, domain="IT", role_groups=["OTHER_IT"],
                        evidence="QA 엔지니어"),
        ))
        svc = _make_service(llm)
...
class TestExperienceDistinction:
    ...
    async def test_required_1yr_vs_preferred_1yr(self):
        p_req = _posting(30, "백엔드 개발자",
                         description="자격요건: 경력 1년 이상", exp="경력 1년 이상")
        p_pref = _posting(31, "백엔드 개발자",
                          description="우대: 1년 이상 경험자 우대", exp="1년 이상 우대")
        llm = _mock_llm(_llm_batch_response(
            _llm_result(30, domain="IT", role_groups=["BACKEND"],
                        exp_req_type="REQUIRED", min_years=1,
                        evidence="자격요건: 경력 1년 이상"),
            _llm_result(31, domain="IT", role_groups=["BACKEND"],
                        exp_req_type="PREFERRED", min_years=None,
...
class TestMultiRoleTracks:
    ...
    async def test_backend_experienced_and_sales_newgrad_tracks_preserved(self):
        p = _posting(40, "2025 공개채용 - 백엔드 개발자 / 영업 담당",
        ...
        llm = _mock_llm(_llm_batch_response(
            _llm_result(40, domain="MIXED", scope="MULTI_ROLE",
...
class TestOpenRecruitmentWithIt:
    ...
    async def test_open_recruitment_with_explicit_it_track(self):
        p = _posting(50, "2025 신입 공채 — IT/비IT",
        ...
        llm = _mock_llm(_llm_batch_response(
            _llm_result(50, domain="MIXED", scope="OPEN_RECRUITMENT",
...
class TestGenericOpenRecruitmentNoItInference:
    ...
    async def test_generic_open_recruitment_no_it_inference(self):
        p = _posting(60, "2025 공개채용", description="다양한 직군 모집 예정. 세부 사항 추후 공개.")
        llm = _mock_llm(_llm_batch_response(
            _llm_result(60, domain="UNKNOWN", scope="UNKNOWN",
                        role_groups=[],
                        tracks=[],
...
class TestInputQuality:
    ...
    async def test_truncated_description_included_in_uncertainty(self):
        ...
        r = resp.results[0]
        assert r.status == "FALLBACK"
        assert any("DESCRIPTION_TRUNCATED" in reason for reason in r.uncertainty_reasons)
...
class TestIdAndHashValidation:
    ...
    async def test_analysis_input_hash_always_echoed_from_request(self):
        ...
        p = ClassifyPostingInput(
            ...
            parsed_roles=[], parsed_experience_level=None,
            parsed_employment_type=None, source_refs=[],
            input_quality=_quality(),
        )
        llm = _mock_llm(_llm_batch_response(
            _llm_result(82, domain="IT", role_groups=["BACKEND"])
        ))
        svc = _make_service(llm)
        resp = await svc.classify(_request(p))
...
class TestErrorHandling:
    ...
    async def test_invalid_json_triggers_fallback_after_retry(self):
        ...
        mock.enabled = True
        mock.call_json = AsyncMock(
            side_effect=LLMClientError("LLM returned non-JSON output: ... First 300 chars: not-json")
        )
    ...
    @pytest.mark.asyncio
    async def test_semantic_contradiction_nonit_with_backend_role_triggers_fallback(self):
    ...
    async def test_prompt_injection_in_description_logged_as_warning(self):
        """Item with injection text is processed; warning is emitted but others succeed."""
        normal = _posting(110, "백엔드 개발자", description="Spring Boot API 개발")
        injected = _posting(111, "채용 담당",
...
class TestConcurrencyAndBudget:
    ...
    async def test_max_concurrency_is_respected(self):
        ...
        async def fake_call_json(*args, **kwargs):
            ...
            async with lock:
                active -= 1
            pid_match = args[1].split('"jobPostingId":')[1].split(",")[0].strip() if '"jobPostingId":' in args[1] else "0"
            try:
                pid = int(pid_match)
            except ValueError:
                pid = 0
            return _llm_batch_response(_llm_result(pid, domain="IT", role_groups=["BACKEND"])), LLMTokenUsage()

        mock = AsyncMock()
        mock.enabled = True
        mock.call_json = fake_call_json

        postings = [_posting(i, f"백엔드 개발자 {i}", description="Spring Boot") for i in range(1, 11)]
...
class TestClassifyEndpoint:
    ...
    async def test_classify_endpoint_happy_path(self, client):
        ...
        with patch("app.api.collections._classify_service") as mock_svc:
            mock_svc.classify = AsyncMock(return_value=ClassifyResponse(
                request_id="ep-req-1",
...
class TestBriefingGraphRegression:
    ...
    async def test_classify_and_briefing_coexist(self, monkeypatch):
        ...
        classify_mock.call_json = AsyncMock(return_value=(
            _llm_batch_response(_llm_result(300, domain="IT", role_groups=["BACKEND"])),
            LLMTokenUsage(),
        ))
...
class TestRuleBasedFallback:
    ...
    def test_aml_backend_developer_is_it(self):
        """'AML' string alone does NOT block IT classification when 'backend' present."""
